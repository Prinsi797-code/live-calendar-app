...
export default function CalendarScreen({ navigation }: any) {
  ...
  const isDarkMode = isDarkMode || (theme === 'system' && systemColorScheme === 'dark');
  ...
  useEffect(() => {
    ...
    if (params.refresh) {
      ...
      // NEW: Handle scroll to current month (from date box click)
      if (params.scrollToCurrentMonth === 'true') {
        console.log('Scrolling to current month');
        const today = new Date();
        const todayString = getLocalDateString(today);

        // Sirf current month pe navigate karo, date select mat karo
